...
export const Profile = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/users/${profile?.id}/update`, {
        method: "PUT",
        headers: {
  ...
  const { data: queriedProfile, isLoading } = useQuery<UserProfile | null>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`http://localhost:3000/api/users/${userId}/profile?full=true`, {
